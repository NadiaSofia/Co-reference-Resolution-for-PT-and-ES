...
class Model(nn.Module):
    ...
    def forward(self, inputs, concat_axis=1):
        pairs = len(inputs) == 11
        if pairs:
            spans, words, spans_bert, single_features, ant_spans, ant_words, ant_spans_bert, ana_spans, ana_words, ana_spans_bert, pair_features = (
                inputs
            )
        else:
            spans, words, spans_bert, single_features = inputs
        ...
        single_input = torch.cat([spans, embed_words, spans_bert, single_features], 1)
        ...
        if pairs:
            ...
            pair_input = torch.cat(
                [ant_spans, ant_embed_words, ant_spans_bert, ana_spans, ana_embed_words, ana_spans_bert, pair_features],
                2,
            )
