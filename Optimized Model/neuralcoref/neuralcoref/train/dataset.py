...
def load_embeddings_from_file(name):
    ...
    print(embed.size())
    print("loading", name + "_vocab.txt")
    with io.open(name + "_vocab.txt", "r", encoding="utf-8") as f:
...
class NCDataset(Dataset):
    def __init__(self, data_path, params, no_targets=False):
        ...
        self.costs = params.costs # N: {"FN": 0.8, "FL": 0.4, "WL": 1.0}
        self.no_targets = no_targets # N: False
        ...
        print("Reading ", end="")
        # N: Read and load npy files in numpy folder
    ...
    def __getitem__(self, mention_idx):
        """
        Return:
            Definitions:
            ...
            if there are some pairs:
                inputs = (spans, words, spans_bert, features, ant_spans, ant_words, ant_spans_bert, ana_spans, ana_words, ana_spans_bert, pairs_features)
                targets = (labels, costs, true_ants, false_ants)
            else:
                inputs = (spans, words, spans_bert, features)
        ...
        """
        features_raw, label, pairs_length, pairs_start_index, spans, words, spans_bert = self.mentions[mention_idx]
        ...
        if pairs_length == 0:
            ...
            spans_bert = torch.from_numpy(spans_bert).float()
        ...
        ant_spans_bert = np.concatenate(
            [self.mentions[idx.item()][6][np.newaxis, :] for idx in pairs_ant_index]
        )

        ana_spans = np.tile(spans, (pairs_length, 1))
        ana_words = np.tile(words, (pairs_length, 1))
        ana_spans_bert = np.tile(spans_bert, (pairs_length, 1))

        ant_spans = torch.from_numpy(ant_spans).float()
        ant_words = torch.from_numpy(ant_words)
        ant_spans_bert = torch.from_numpy(ant_spans_bert).float()

        ana_spans = torch.from_numpy(ana_spans).float()
        ana_words = torch.from_numpy(ana_words)
        ana_spans_bert = torch.from_numpy(ana_spans_bert).float()
...
class NCBatchSampler(Sampler):
    ...
    def __init__(self, mentions_pairs_length, batchsize=600, shuffle=False):
    ...
    def save_batch_sizes(self, save_file=BATCH_SIZE_PATH):
...
def padder_collate(batch):
    ...
    max_pairs = (
        max(len(t) for t in transposed_inputs[4]) if len(transposed_inputs) == 11 else 0
